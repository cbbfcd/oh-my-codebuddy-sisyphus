...
function peekRecentOutboxMessages(
  ...
  const outboxPath = join(getCodebuddyConfigDir(), 'teams', safeName, 'outbox', `${safeWorker}.jsonl`);
...
export function getTeamStatus(
  ...
  const workers: WorkerStatus[] = mcpWorkers.map(w => {
    ...
    const provider = w.agentType.replace(/^(?:mcp|tmux)-/, '') as 'claude' | 'codex' | 'gemini' | 'codebuddy';

...
```
/oh-my-codebuddy:sciomc <goal>                    # Standard research with user checkpoints
/oh-my-codebuddy:sciomc AUTO: <goal>              # Fully autonomous until complete
/oh-my-codebuddy:sciomc status                    # Check current research session status
/oh-my-codebuddy:sciomc resume                    # Resume interrupted research session
/oh-my-codebuddy:sciomc list                      # List all research sessions
/oh-my-codebuddy:sciomc report <session-id>       # Generate report for session
```

### Quick Examples

```
/oh-my-codebuddy:sciomc What are the performance characteristics of different sorting algorithms?
/oh-my-codebuddy:sciomc AUTO: Analyze authentication patterns in this codebase
/oh-my-codebuddy:sciomc How does the error handling work across the API layer?
```
...
Task(subagent_type="oh-my-codebuddy:scientist", model="haiku", prompt="[RESEARCH_STAGE:1] Investigate...")

// Stage 2 - Standard analysis
Task(subagent_type="oh-my-codebuddy:scientist", model="sonnet", prompt="[RESEARCH_STAGE:2] Analyze...")

// Stage 3 - Complex reasoning
Task(subagent_type="oh-my-codebuddy:scientist", model="opus", prompt="[RESEARCH_STAGE:3] Deep analysis of...")
```
...
Task(subagent_type="oh-my-codebuddy:scientist", model="sonnet", prompt="
...
4. **Cancellation:** `/oh-my-codebuddy:cancel` or "stop", "cancel"

### AUTO Mode Example

```
/oh-my-codebuddy:sciomc AUTO: Comprehensive security analysis of the authentication system
...
Task(subagent_type="oh-my-codebuddy:scientist", model="haiku", prompt="[STAGE:1] Analyze src/api/...")
Task(subagent_type="oh-my-codebuddy:scientist", model="haiku", prompt="[STAGE:2] Analyze src/utils/...")
Task(subagent_type="oh-my-codebuddy:scientist", model="haiku", prompt="[STAGE:3] Analyze src/components/...")
```
...
Task(subagent_type="oh-my-codebuddy:scientist", model="sonnet", prompt="[HYPOTHESIS:A] Test if caching improves...")
Task(subagent_type="oh-my-codebuddy:scientist", model="sonnet", prompt="[HYPOTHESIS:B] Test if batching reduces...")
Task(subagent_type="oh-my-codebuddy:scientist", model="sonnet", prompt="[HYPOTHESIS:C] Test if lazy loading helps...")
```
...
Task(subagent_type="oh-my-codebuddy:scientist", model="opus", prompt="
[CROSS_VALIDATION]
...
| Command | Action |
|---------|--------|
| `/oh-my-codebuddy:sciomc status` | Show current session progress |
| `/oh-my-codebuddy:sciomc resume` | Resume most recent interrupted session |
| `/oh-my-codebuddy:sciomc resume <session-id>` | Resume specific session |
| `/oh-my-codebuddy:sciomc list` | List all sessions with status |
| `/oh-my-codebuddy:sciomc report <session-id>` | Generate/regenerate report |
| `/oh-my-codebuddy:sciomc cancel` | Cancel current session (preserves state) |
...
Optional settings in `.codebuddy/settings.json`:
